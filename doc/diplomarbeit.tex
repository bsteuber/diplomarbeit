...
\end{code}
übersetzt werden, woraus ein -- derzeit fiktiver -- Latex-DSL-Compiler den Latex-Ausgabecode
\begin{code}
...
(autoMac compSlide slide
  (liftA2 genSlide compSymbol (many compAuto))
  (where (= (genSlide title contents)
            (' (env (frame (, title))
                 (,@ contents))))))

(autoMac compList list
  (>>^ (many compSymbol) genList)
  (where 
    (= (genList items)
       (' (env itemize
            (,@ (map genItem Items)))))
