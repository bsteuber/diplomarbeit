...
erneut expandiert wird. Ein Makro kann immer nur genau einen Ausdrick
zurückgeben, es ist also nicht möglich keinen oder mehrere \sexps{} zu
erzeugen. Dies ist in der Praxis allerdings kein großes Problem, da in
Lisp mit dem \icode{do}-Operator beliebig viele Ausdrücke zu einem
zusammengefasst werden können. Lässt sich kein Makro zu einem Ausdruck
...
(def e 5)  
\end{code}
kompiliert. Stattdessen müsste man den Operator \icode{do} benutzen:
\begin{code}
(def a 1)
(do
  (def b 2)
...
(def e 5)  
\end{code}
Um ein Makro wie \icode{defall} auch für Zielsprachen ohne \icode{do} zu
...
sein. Da MagicL in Haskell geschrieben ist, ist eine Verwendung von
Haskell als Meta-Sprache naheliegend. Allerdings ist Haskell nicht
...
die automatisch nach Haskell kompiliert wird. Also muss MagicL selbst
bereits generative Meta-Programmierung für Haskell benutzen.

\lsection[reqs:call]{Aufruf von Templates}

Wie in Lisp Auch die Möglichkeit, die für einen \sexp{}
...
Wie in \sref{template:crit:gentemp} beschrieben
...
\bf Template-Aufruf          & Makro                         & Makro                  \\\hline      
\bf Komponententrennung      & möglich                       & möglich                \\\hline      
...
Darüber hinaus wird eine theoretische Fundierung für die Semantik der
Kombination dieser Verarbeitungseinheiten gefordert. Diese wird in
MagicL durch die Anwendung kategorientheoretischer Konzepte erreicht,
welche im folgenden Kapitel beschrieben werden.
