...
stärksten überzeugen. Lisp-Dialekte stellen jedoch kein universelles
\cgen{}swerkzeug da, da diese ausschließlich Lisp-Code generieren
können. Die Übertragung auf beliebige Zielsprachen ist daher die
wichtigste der in \cref{reqs} besprochenen Anforderungen. Auch externe
Eingabeformate sollten durch die Konstruktion spezifischer Parser
unterstützt werden können. Darüber hinaus wurden gegenüber Lisp weitere
Verallgemeinerungen gefordert: So sollten neben \sexps{} auch Strings
und Objekte als Modelle verwendet werden können, obwohl der Focus auf
\sexps{} bleibt. Verarbeitungseinheiten wie Makros, Parser und anders
implementierte Compiler sollen sowohl explizit aufrufbar sein als auch
über ein Pattern-Matching, welches über den Knotennamen hinausgeht. Auch
``lokale Makros'', welche nur im Kontext anderer Makros ausgewertet
werden, sollten realisierbar sein, um auf ein komplexes ``Code-Walking''
wie in Lisp üblich möglichst verzichten zu können.

Eine weitere Forderung war die Möglichkeit, Verarbeitungseinheiten nach
dem Baukastenprinzip in einer klaren Semantik miteinander auf
verschiedene Weisen kombinieren zu können. Die theoretische Fundierung
dieser Semantik liefert die Kategorientheorie, deren Grundzüge in
\cref{cats} vorgestellt wurden. Kategorien bestehen aus Morphismen
zwischen Objekten - wobei beide Begriffe axiomatisch definiert und somit
zunächst bedeutungsneutral sind. Die häufigste Interpretation für
Morphismen und Objekte in der Mathematik sind Funktionen und Mengen, im
Rahmen dieser Arbeit kommen jedoch ausschließlich Verarbeitungsprozesse
und Typen vor. Kategorien können anschaulich als Diagramme visualisiert
werden. Innerhalb einer Kategorie können neben der Komposition Produkte
und Coprodukte verwendet werden, um einfache Einheiten zu komplexeren
zusammenzusetzen. Auch ganze Kategorien können über Funktoren
aufeinander abgebildet werden. Monaden sind spezielle Funktoren auf
einer Kategorie, die eine spezielle, durch sogenannte natürlichen
Transformationen beschriebene Struktur besitzen.

Die Sprache Haskell, die in \cref{haskell} präsentiert wurde, bietet in
ihren Bibliotheken die Typklassen \icode{Arrow} und \icode{Monad},
welche eine programmiersprachliche Umsetzung spezieller Kategorien und
Monaden darstellen. Dies ist einer der Gründe, welhalb MagicL in Haskell
...
\lsection[end:disc]{Diskussion und Ausblick}
  ...
  \item MagicL sauberer und klarer als Lisps
    ...
    \end{itemize}
  \item Einschränkungen im Typsystem
  ...
  \item[$\Rightarrow$] Hier könnte \cgen{} helfen
    ...
    \item Kleine DSL für \icode{code}-Konstruktion (kein \icode{text}
      ...
      Srpachen helfen
    \item Compiler (merge autoMac und defmacro, gensym -> Zustände)
    \end{itemize}
...
\item \sexp{}-IDE
  \begin{itemize}
  \item Struktureller Editor, Makrobasiertes Theming
