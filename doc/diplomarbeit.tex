...
spezielle Eigenschaften eines Modells festzuhalten. Unter anderem werden
Attribute auch für objektorientierte Programmierung verwendet. Manche
Modellrepräsentationen stellen direkt Attribute bereit, andere wie
\sexps{} können diese beispielsweise über Unterknoten simulieren.
...
dabei allerdings beliebige Zielsprachen unterstützen. Das Level von
Zeichenketten soll für die Metaprogrammierung möglichst keine Rolle
spielen - stattdessen sollen als Modellrepräsentationen sowohl \sexps{}
- welche so einfach wie möglich definiert werden - als auch getypte
Objekte verwendet werden können. Dies gilt auch für die Ausgabetypen von
Makros, deren Pattern-Matching im Gegensatz zu Lisp nicht nur den
Knotennamen, sondern beliebige \sexps{} unterstützen soll. Der
Kontrollfluss von Makroaufrufen soll feiner als in Lisp gesteuert werden
können, um "`lokale Makros"' zu ermöglichen. Neben der textuellen
Repräsentation von \sexps{} sollen prinzipiell auch andere Datenformate
unterstützt werden, weshalb eigene Parser erstellt und eingebunden
werden können sollen. Eine allgemeine Schnittstelle soll schließlich die
Integration von Verarbeitungseinheiten wie Makros, Parsern und durch
Funktionen implementierte Compiler ermöglichen.

Darüber hinaus ist eine theoretische Fundierung für die Semantik der
Kombination dieser Verarbeitungseinheiten. Diese wird in MagicL durch
durch die Anwendung kategorientheoretische Konzepte erreicht, welche im
...
Identitätsmorphismus. Auch diese Gleichungen ließen sich als kommutative
...
bis hin zu vollständigen Programmiersprachen. Tools, in denen Meta- und
...
Von den untersuchten Tools konnten Lisp-Makros aufgrund ihrer
...
und für den Eingabestream verwendet wird. Darüber hinaus wurde eine mit
EBNF vergleichbare, jedoch allgemeinere und erweiterbare
Kombinator-Bibliothek entwickelt, mit der aus Parser-Arrows funktional
...
können. Ähnlich wie die Kombinator-Bibliothek für Parser kann auch die
...
beliebige Programmier- und Auszeichnugssprachen eingelesen oder
...
Kombinatorbibliothek, die mächtiger ist ist als EBNF oder XPath-Patterns
in XSLT.
...
Einsatz in konkreten Projekten gedacht ist. Dieser Abschnitt soll daher
einen Überblick geben über all das, was derzeit in MagicL nicht oder
nicht optimal möglich ist. Dabei wird diskutiert, welche fundamentalen
Designentscheidungen in einem möglicherweise Nachfolger anders getroffen
werden könnten und an welchen Stellen noch Nachbesserungen oder
Erweiterungen möglich sind. Auch sollen Ideen für \sexp{}-gestützte
Modellierung und Verarbeitung aufgezählt werden, die hier nicht weiter
verfolgt wurden, aber interessante Forschungsrichtungen für die Zukunft
darstellen könnten.

\lsubsection[end:disc:realworld]{Nutzung in der realen Welt}

Generell lag bei der Implementation von MagicL die Prioritär auf
theoretischer Korrektheit und klarer Semantik, jedoch weniger auf
konkreten Anwendungen. Dies spiegelt sich auch in der Wahl von Haskell
und Kategorientheorie als Basistechnologien wieder, welche zwar
mathematisch elegant sind, sicherlich jedoch die meisten Anwender
abschrecken würden. Hierfür könnte eine einfachere Compiler-DSL nützlich
sein, die ohne mathematisches Hintergrundwissen benutzbar ist. Diese DSL
könnte dann entweder in die bestehende Arrow-Implementation überführt
oder komplett anders - und dabei eventuell effizienter als die
verschachtelten Parser-Funktoren, die vergleichsweise langsam laufen -
realisiert werden.

Ein weiterer Aspekt der Realweltnutzung liegt in der Integrierbarkeit in
bestehende Systeme. MagicL-Compiler können mit einem installierten
...
beliebigen Build-Scripten aufgerufen werden können. Dennoch wären
Interoperationalität und Portabilität bei einer Umsetzungssprache wie
Java deutlich höher.

\lsubsection[end:disc:compiler_defs]{Vereinfachung von
  Compiler-Definitionen} 

Auch im Rahmen der Arrow-Architektur 

So löst MagicL zwar prinzipiell das Problem baukastenartiger,
theoretisch fundierter, beliebig typisierter und sprachunabhängiger
\cgen{}; wendet allerdings selbst zu wenig \cgen{} an, um das Optimum an
Bedienbarkeit zu erreichen.

Beispielsweise ist die Definition von
typisierten Compilern über die \icode{Compilable}-Schnittstelle
\sref{sexphs:code} in Haskell (und damit auch in \sexp{}-Haskell)
relativ umständlich. Hierfür könnte eine neue Modellierungssprache
bereitgestellt und nach \sexp{}-Haskell kompiliert werden.

Auch die ausschließlich \sexp{}-basierte Verarbeitung mittels
\sexp{}-Compiler ist sicherlich noch nicht optimal, da als Parser
spezifizierte Makros noch immer mehr Aufwand bedeuten als in
Lisp-Dialekten. Für exakte Lisp-Makros könnte zwar einfach ein
\icode{defmacro} bereitgestellt werden, dies würde allerdings die
Allgemeinheit des Parser-Ansatzes zerstören. Auto-Makros sind der
Versuch eines Kompromisses aus Universalität und Lisp-Komfort, bieten
allerdings mit Sicherheit nicht das Optimum in beiden Hinsichten. Eine
interessante Erweiterung wären eventuell ``lokale Auto-Makros'', die
zwar in jeder Verschachtelungstiefe, dennoch nur innerhalb eines oder
mehrerer speziellen Makros ausgewertet wird. Dennoch kann die Frage,
welches Meta-Sprachdesign die volle Flexibilität kategorientheoretischer
Parser mit einer einfachst-möglichen Nutzbarkeit verbindet, an dieser
Stelle nicht umfassend beantwortet werden.

\lsubsection[end:disc:new_dsls]{Weitere DSLs}

Eine weitere \sexp{}-DSLs könnte die Benutzung des typisierten
Code-Modells über Quasiquotes erleichtern. Darüber hinaus könnte neben
Haskell die \sexp{}-basierte Erzeugung beliebiger weiterer Sprachen
direkt unterstützt werden - seien es Java, PHP, Latex oder auch
VHDL. Denn erst das Vorhandensein dieser Backends macht ein Framework
auch praktisch universell.

\lsubsection[end:disc:generalize]{Theoretische Verallgemeinerungen}

Beispielsweise ermöglicht die generische Compiler-Schnittstelle zwar das
automatische Finden von Compilern, nicht aber die selbstständige Suche
von Pfaden über mehrere Stationen - dies würde nur funktionieren, wenn
das Haskell-Typsystem eine Prolog-artige Tiefensuche\cite{Prolog}
anbieten würde oder eine künftige Implementation nicht über
Haskell-Typen, sondern Datenstrukturen erfolgen würde.

Sexp-Typ verallgemeinern? Sexp = Tree String, Char? Parser auf Symbolen!
Gemischte Bäume, vgl. Quasiquote - dynamisch typisiert?

Parser-Templates? Prolog-Makros (bidirektional?)
Umgang mit Mehrdeutigkeiten

\lsubsection[end:disc:ide]{Entwicklungsumgebung}

\sexp{}-IDE
 Struktureller Editor, Makrobasiertes Theming
 Modellspezifische GUI-Plugins
 Netzwerktransparenz und synchrone Bearbeitung
 Type-Time Inference

\lsubsection[end:disc:db]{Datenbank}

Verteilte Sexp-Datenbank
 Schema-frei
 Versionierung 
 Diff (evtl. über IDE-Protokoll)
 Merge für Teamarbeit und passive \cgen{}
 Echtzeit?

\lsubsection[end:disc:networking]{Vernetzung}
