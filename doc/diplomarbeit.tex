...
Template based code generation is ubiquitous throughout computer science
and used in many different forms, for example the C preprocessor, PHP or
XSL Transformations -- just to mention a few. However, most existing
tools are quite cumbersome to use and error prone due to their use of
syntax and data structures. An alternative, yet almost forgotten
approach is taken by the Lisp language family languages, where \sexps{} are
used as a simple and universal data structure for code representation,
being processed by macros that can be regarded as ``lightweight
compilers''.
...
Haskell which employs an architecture inspired by category theory. This
allows to create and combine parsers, generators as well as other kinds
of compilers by the means of arrows that can have \sexps{} as well as
strings and typed objects as input and output types.  On top of this, a
DSL -- which is itself denoted in \sexps{} -- for defining compilers is
constructed. The compiler DSL includes a Haskell DSL, which is a
complete functional programming language, as well as a quasi-quote
operator and a generalized macro system for easy processing of \sexps.
...
Diese Art der \mprog{} ist in der Informatik omnipräsent,
beispielsweise kann jeder Compiler als Metaprogramm gesehen werden. Die
...
15-jähriges Bestehen\footnote{Der Standarisierungsprozess von UML begann
 bereits 1994, als Gary Booch und Jim Rumbaugh mit der Vereinheitlichung ihrer
...
fällt auch im WWW eine immer größer werdende Rolle zu: Ob
...
anderen Notationen wie XML verglichen.
...
führt deshalb in die Kategorientheorie ein und erklärt später
benötigte Strukturen wie Funktoren, Arrows oder Monaden.
...
Namespace-Präprozessor bereitstellt, der die Doppelpunkte korrekt
...
große Rolle bei \cgen{} spielen. Ein Template legt die statischen
...
Metainformationen wie Typen über Code in derselben Datei abgefragt
...
\bf Zielsprache             & Haskell                     &beliebig                    &Lisp                  \\\hline      
...
lediglich eine globale Zuordnung von Namen zu Makros besteht\footnote{Es
  ...
  für einen bestimmten lexikalischen Skopus verfügbar zu machen,
...
Strukturen sind Kategorien. Kategorientheorie spielt dank seiner
...
lediglich ein Alias für Listen von Buchstaben:
\begin{DIFnomarkup}\begin{code}
type String = [Char]
\end{code}\end{DIFnomarkup}
...
weggeschmissen, im Erfolgsfall die Eingabe. Anschließend wird im
...
werden. Anschließend wird ein Makro mit dem Haskell-Namen
\icode{compAuto} erzeugt, welches alle gefundenen Namen an
\icode{lispTraverse} übergibt. Da das Scannen von Auto-Makros bereits
vor der normalen Makroexpansion von \icode{autoMac} geschieht, kann
...
einen (Quasi-)Quote-Operator \icode{'} sowie ein Lisp-ähnliches
...
Die Zeichenketten-Ebene muss vom Metaprogrammierer allerdings nur für
die Unterstützung neuer externer Ein- und Ausgabeformate betreten
werden. Im Idealfall\footnote{Idealfall meint hier, dass ausschließlich
  \sexp{}-DSLs verwendet werden, alle Backends für zu generierende
  Zielsprachen bereits vorhanden sind und die Verarbeitungsalgorithmen
  nicht so komplex sind, dass getypte Objekte benötigt werden.} können
  Compiler komplett auf der einfachen \sexp{}-Ebene spezifiziert werden.
...
übergeben werden müssen, um denselben Effekt zu erzielen. Es wäre daher
...
\sexp{} ergibt wie \icode{(List a b)} -- einen derartigen Präprozessor
...
Zum Abschluss werden die wichtigsten Aspekte dieser Arbeit noch einmal
...
universellen Frameworks zur \cgen{}, welches möglichst
...
verschiedenen Systemen verwendet, allerdings gibt es keines, das diese
gleichzeitig anbietet. Die Bandbreite der einzelnen Metasprachen reicht
von kaum mehr als dem bloßen Template-Formalismus aus Quasiquote und
Unquote bis hin zu vollständigen Programmiersprachen. Werkzeuge, in
denen Meta- und Zielsprache übereinstimmen können,
erlauben prinzipiell auch die Generierung von Meta-Code selbst, woraus
sich interessante Möglichkeiten ergeben. Hier kann es allerdings zu
einer Mehrdeutigkeit bei der Auswertung von Unquotes kommen, die meist
nur umständlich umgangen werden kann. Das Aufrufen von Templates
verläuft meist explizit wie bei einem Funktionsaufruf. Alternativ treten
auch Makros auf, die implizit über den Namen des zu verarbeitenden
Ausdrucks zugeordnet werden. XSLT unterstützt hier sogar ein
...
universelles \cgen{}swerkzeug dar, da diese ausschließlich Lisp-Code
...
externe Eingabeformate sollten durch die Konstruierbarkeit
...
obwohl der Fokus auf \sexps{} bleibt. Makros sollten sowohl explizit
aufrufbar sein als auch über ein Pattern-Matching gegen den zu
...
verschiedene Weisen und in einer Semantik kombinieren zu können. Das
theoretische Fundament dieser Semantik liefert die Kategorientheorie,
...
natürliche Transformationen beschriebene Struktur besitzen.
...
sind Arrows oder lassen sich in Arrows konvertieren, und
...
implizit über ihre Ein- und Ausgabetypen angesprochen werden.
...
Metaprogrammiersprache für Compilerdefinitionen gedacht ist und darüber
...
Quasiquote-Operator für \sexps{} wie in Lisp üblich entwickelt,
...
Metaprogramme noch nicht so kurz formuliert werden können wie in
...
Dieser Abschnitt soll einen Überblick geben über weitere Möglichkeiten
um MagicL zukünftig zu verbessern. Dabei wird diskutiert, welche
Entwurfsentscheidungen in einem möglichen Nachfolger anders getroffen
werden könnten und an welchen Stellen noch Nachbesserungen oder
Erweiterungen möglich sind. Darüber hinaus sollen Ideen für
\sexp{}-gestützte Modellierung und Verarbeitung aufgezählt werden, die
hier nicht weiter verfolgt wurden, aber interessante
...
komplett anders realisiert werden -- und dabei eventuell effizienter
sein als die verschachtelten Parser-Funktoren, die vergleichsweise
langsam laufen.
...
Haskell-Typen, sondern eigene Datenstrukturen erfolgt. Eine Umsetzung in
einer dynamisch typisierten Sprache wie Clojure \cite{Clojure} könnte
hilfreich sein.
...
Roundtrip-Engineering \cite{Roundtrip} ermöglichen. Bei Mehrdeutigkeiten
...
Entwicklungsszenarien könnten \sexps{} Vorteile gegenüber textbasierten
...
könnten so immer abstraktere und einfachere Notationen für Domänen
...
für derartige DSL-Compiler einen ersten Schritt in diese Richtung.
