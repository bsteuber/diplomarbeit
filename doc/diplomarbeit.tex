...
In \cref{template} geht es um Templates, welche in der einen oder
anderen Form von praktisch allen \cgen{}swerkzeugen verwendet
werden. Hier wird eine Reihe bestehender Tools mit teilweise sehr
unterschiedlichen Verwendungsweisen von Templates vorgestellt und
evaliert - unter anderem auch Lisp-Makros, die als eine spezielle Form
von Templates aufgefasst werden.
...
der Mathematik ein, mit deren Hilfe - neben vielen anderen
...
Funktoren eine Kategorie für Parser konstruieren lässt.

\cref{sexpproc}

\cref{magicl}

\cref{end} fasst die Ergebnisse zusammen und diskutiert das entstandene
Framework. Anschließend wird ein Ausblick auf weitere
Verbesserungsmöglichkeiten und Fortsetzungsmöglichkeiten, die nicht im
Rahmen dieser Arbeit behandelt werden konnten.
...
\todo{sexps in lisp enthalten zahlen, strings und symbole}
...
\lsubsection[template:crit:lang]{Meta-Programmiersprache}
...
\lchapter[reqs]{Anforderungsanalyse}
...
einzubauen, wie \sref{cats_hask:monads} erklärt.
...
\sref{cats:monads} und \sref{cats_hask:monads}. Auch
...
\lsection[cats_hask]{Kategorien in Haskell}
...
\lsubsection[cats_hask:arrows]{Arrows}
...
\lsubsection[cats_hask:coproducts]{Coprodukte: Die Klasse \texttt{ArrowChoice}}
...
\lsubsection[cats_hask:functors]{Funktoren}
...
\lsubsection[cats_hask:monads]{Monaden}
...
zuständig ist (siehe \sref{cats_hask:monads:examples:io}). Diese
...
\lsubsubsection[cats_hask:monads:examples]{Beispiele für Monaden}

\lparagraph[cats_hask:monads:examples:maybe]{\icode{Maybe}}
...
\lparagraph[cats_hask:monads:examples:io]{\icode{IO}}
...
\icode{first}-Funktion aus \sref{cats_hask:arrows}.
...
Konzepte (in \sref{cats_hask} beschrieben), mit deren Hilfe
