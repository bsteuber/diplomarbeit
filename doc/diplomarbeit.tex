...
von \sexps{}. Hierzu gehören ein String-Parser
(\sref{arrowparser:sexp}) und ein Pretty-Printer
(\sref{magicl:code:prettysexp} für \sexp{}-Syntax. Darüber hinaus werden
einige spezielle Kombinatoren für das Verarbeiten von \sexps{} durch
Parser bereitgestellt. Beispielsweise kann mit \icode{macro} ein
\sexp{}-Parser definiert werden, der wie ein Lisp-Makro nur aktiv wird,
wenn das erste Element eines Ausdrucks mit dem Knotennamen
übereinstimmt. Dies wird in \sref{arrowparser:sexpparser} besprochen.
...
Aufgrund der Einfachheit der \sexp{}-Struktur und der praktischen
...
\todo{mehr Top-Down argumente, aufzählen aller Standardcompiler}
...
und mächstiger sind als gewöhnliche Lisp-Makros, letztere aber dennoch
...
Sprache. Die größere Allgemeinheit geht allerdings derzeit mit etwas
erhöhtem Aufwand für die einfachen, in bestehenden Lisps unterstützen
Fälle einher. Beispielsweise könnte ein Makro, welches eine
Foliendefinition einer Slide-DSL in eine Latex-DSL übersetzt, die
Eingabe
\begin{code}
(slide MySlideTitle
  (image image.png)
...
in die Ausgabe
\begin{code}
(environment (frame MySlideTitle)
  (image image.png)
  (items one two three))
\end{code}
...
definiert werden, wo explizite Aufrufe von \icode{take} bzw. \icode{many
  take} und eine Umleitung über \icode{liftA2} für denselben Effekt
benötigt werden. Es wäre daher durchaus wünschenswert, die Compiler-DSL
noch weiter zu vereinfachen, allerdings ist es nicht klar wie genau dies
aussehen soll. Es könnte beispielsweise ein komplett mit Common Lisp
übereinstimmendes \icode{defmacro} angeboten werden, welches kurze
Definitionen ermöglicht, im Gegenzug aber an Allgemeinheit
verliert. Welche Sprache wirklich den besten Kompromiss zwischen
Allgemeinheit und Programmkürze ermöglicht bleibt daher eine offene
Frage, die sich vermutlich erst nach häufiger Verwendung näher
...
sind verschachtelte Quasiquotes teilweise sehr schwierig zu
verstehen. Beispielsweise könnte ein Makro, welches ein anderes Makro
definiert, in Common Lisp durch
\begin{code}
(defmacro x ()
  `(defmacro y (a)
      `(foo ,a)))
...
(defmacro x (a)
  `(defmacro y ()
      `(foo ,',a)))
\end{code}
Aufgrund der Lisp-Evaluationsregeln muss der Konstrukt \icode{,',a}
verwendet werden. Es gibt noch weitere Fälle dieser Art, und nur die
wenigsten Lisp-Programmierer verstehen all diese Konstrukte sofort. Mit
dem MagicL-Quasiquote wird das erste Beispiel\footnote{Hier wurde für
  die bessere Vergleichbarkeit weiterhin \icode{defmacro} verwendet,
  obwohl dies in MagicL nicht definiert ist.} durch
\begin{code}
(defmacro x ()
  (' (defmacro y (a)
       ('1 (foo (,1 a))))))
...
(defmacro x (a)
  (' (defmacro y ()
       ('1 (foo (, a))))))
\end{code}
ausgedrückt, was deutlich intuitiver ist.
