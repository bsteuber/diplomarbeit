...
\lsection[sexpproc:sexphs]{Erzeugung von Haskell-Code}
...
übersetzt. Der Compiler dient gleichzeitig als Anschauungsbeispiel für
die Benutzung eines typisierten Zwischenmodells in MagicL - alternativ
hätte dieser auch untypisiert von \sexps{} nach \sexps{} mit den in
\sref{sexpproc:gen} beschriebenen Hilfsmitteln erstellt werden können,
was eine kürzere, aber fehleranfälligere Implementation mit sich geführt
hätte.

\lsubsection[sexpproc:sexphs:model]{Typisiertes Haskell-Modell}

Als erster Schritt wird das Haskell-Modell entwickelt.  Eine
Haskell-Codedatei enthält eine optionale Modulbeschreibung sowie eine
Liste von Toplevel-Definitionen:
\begin{code}
data Haskell = Haskell (Maybe Module) [Toplevel]  
\end{code}
Die Definition von \icode{Module} wird der Übersichtlichkeit halber
ausgelassen. Toplevel-Forms können Typ- oder Funktionsdeklaration oder
die Definition neuer Typaliase, Datentypen, Klassen oder Instanzen sein:
\begin{code}
