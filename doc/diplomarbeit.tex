...
definiert werden. Mittels Klammern kann ein Infixoperator in eine
...
wird, die vom Parameter 2 subtrahiert. Für die obige, sogenannte
punktfreie Art der Funktionsdefinition mittels Currying ist auch der
Kompositionsoperator \icode{.} sehr nützlich:
\begin{code}
(.) :: (b -> c) -> (a -> b) -> (a -> c)
(g . f) x = g (f x)
...
\icode{Nothing} entspricht \icode{null} in anderen Sprachen.
 ...
 Datentypen, newtype (Abstraktion f. Typklassen), type (Typvariablen)
