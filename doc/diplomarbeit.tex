...
\lsubsection[basic:cats:nats]{Natürliche Transformationen}
...
\lsubsection[basic:cats:monads]{Monaden}
...
Verknüpfen. Haskell (siehe \sref{basic:haskell}) benutzt Monaden unter
anderem, um Seiteneffekte in eine normalerweise pur funktionale Sprache
einzubauen, wie \sref{magicl:cats:monads} erklärt.
...
Konzepte (in \sref{magicl:cats} beschrieben), mit deren Hilfe
...
\lsection[magicl:cats]{Kategorien in Haskell}
...
\lsubsection[magicl:cats:arrows]{Arrows}
...
\lsubsection[magicl:cats:functors]{Funktoren}
...
\sref{basic:cats:monads} heißt hier \icode{return}, statt $\mu$ wird der
Operator \icode{>>=} %<<
...
zuständig ist (siehe \sref{magicl:cats:monads:examples:io}). Diese
...
\lsubsubsection[magicl:cats:monads:examples]{Beispiele für Monaden}

\lparagraph[magicl:cats:monads:examples:maybe]{\icode{Maybe}}
...
\lparagraph[magicl:cats:monads:examples:io]{\icode{IO}}
...
Haskell besitzt mit \icode{Parsec}\cite{Parsec} bereits eine sehr
performante und praktische Parser-Kombinator-Bibliothek.
...
\icode{first}-Funktion aus \sref{magicl:cats:arrows}.
