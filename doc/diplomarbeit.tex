...
\sees{model:sexp} in einer untypisierten Baumstruktur repräsentiert, so
dass Zeichenketten weder eingelesen noch erzeugt werden müssen, wodurch
ein großer Teil des Generators bereits wegfällt - denn das Verarbeiten
und Erzeugen von Bäumen ist simpler und weniger fehlerbehaftet. \sexps{}
...
können. Damit wird es sogar möglich, Haskell mittels Typinferenz
selbstständig den passenden Compiler für eine Aufgabe auswählen zu
lassen.

Die hier entwickelten Parser können nicht nur auf Zeichenketten, sondern
auf beliebigen Listen arbeiten. \cref{sexpcomp} zeigt, wie damit die
...
Prolog\cite{NorvigProlog} - selbstverständlich in \sexp{}-Syntax -
...
mit EBNF vergleichbare Grammatik auf \sexps{} vorstellbar.
...
\lchapter[magicl]{Komponenten von MagicL}

Die im letzen Kapitel vorgestellten Arrow-basierten Parser und Makros
sowie die im folgenden damit implemtierten Sprachen sind das Herzstück
von MagicL. Darüber hinaus gibt es allerdings noch weitere Komponenten,
...
Code daher an einen Formatter wie Jaloppy\cite{TODO} übergeben werden -
...
Makro\footnote{Wünschenswert wäre ein System, dass automatisch den
...
\icode{comp} referenziert. Alternativ können aber selbstverständlich
auch benannte Makros verwendet werden.
...
ebenfalls auf \sexp{}-Basis erfolgen soll, wird in diesem Abschnitt ein
Compiler vorgestellt, welcher eine in \sexps{} definierte, einer
Haskell-Teilmenge entsprechende Sprache in legalen Haskell-Quellcode
übersetzt.

Bei dem Entwurf eines neuen Compilers stellt sich grundsätzlich immer
die Frage, ob die \sexp{}-basierten Eingabesprache vor der weiteren
Verarbeitung in ein typisiertes Modell überführt werden soll. Dies
bedeutet zwar einen erhöhten Entwicklungsaufwand\footnote{Die zukünftige
  Entwicklung einer geeigneten DSL zur Modellbeschreibung, aus der
  sowohl das typisierte Modell als auch Compiler generiert werden,
  könnte den Entwicklungsaufwand wieder reduzieren.}, führt aber zu
früherer Fehlererkennung in Compilern. Der Haskell-Generator beschreitet
diesen Weg und dient damit gleichzeitig als Anschauungsbeispiel für die
Benutzung typisierter Modelle. Die später in diesem Kapitel vorgestellte
DSL für Lisp-Makros hingegen dient als Beispiel für rein
\sexp{}-gestützte Kompilation.

\lsection[sexphs:spec]{Spezifikation}

\todo{DSL selbst mehr vorstellen (=, where, Operatoren, Klassen usw.)}
...
exportierter Symbole sowie einer Liste von Imports, wobei es vier Arten
gibt, ein Modul zu importieren: Standardimport, benannter Import mit
einem Kurznamen, ausschließlicher Import bestimmter Symbole sowie der
Import aller nicht aufgezählten Symbole:
\begin{code}
...
\icode{do}-Ausdruck für Monaden, eine Typdeklaration oder ein Pattern
sein. Die Parameterliste anonymer Funktionen benutzt ebenfalls Patterns.
\begin{code}
...
\lsection[sexphs:comp]{Compiler-Implementation}

Compiler für größere, aus vielen Datentypen zusammengesetzte Modelle
...
importiert, während aus \icode{Prelude} die Funktion \icode{id} und den
...
Der Parser erzeugt daraus nun ein typisiertes Objekt:
\begin{code}
(Module "MyModule"
         Nothing
...
\end{code}
Dieses Objekt kann nun von weiterverarbeitet werden - konkret wird es in
den Datentyp \icode{Code} überführt, welcher in \sref{magicl:code}
erläutert wird. Aus diesem wird dann automatisch Quelltext erzeugt:
\begin{code}
...
Die in diesem Abschnitt vorgestellte Modellverarbeitung schließt
Typfehler aus und erlaubt die effiziente Durchführung beliebiger
Berechnungen auf den Daten. Allerdings ist sie auch mit einem erhöhten
Aufwand verbunden, der nicht für jede kleine Sprache oder
Spracherweiterung sinnvoll erscheint. Deswegen stellt MagicL alternativ
die Möglichkeit der direkten, untypisierten Verarbeitung von \sexps{}
bereit, welche in den folgenden Abschnitten präsentiert wird.
...
Während das Einlesen von \sexps{} dank der in \sref{arrowparser:sexpparser}
vorgestellten Funktionen bereits recht komfortabel geht, ist die
Erzeugung von Instanzen des \icode{Sexp}-Datentyp zunächst
aufwändig. Soll beispielsweise eine Funktion \icode{genUser userName
  userEmail} mit den Parametern \icode{\"Harald\"} und
\icode{\"Harry@baz.com\"} aufgerufen den Ausdruck
\begin{code}
...
speziell auf MagicL zugeschnitten sein soll und keine bloße Kopie
bestehender Lisp-Quasiquotes. Im folgenden werden deshalb die
Anforderungen diskutiert und das gewünschte Verhalten festgelegt.
...
\end{code}
Dies sollte von einem geeigneten Compiler in obige, umständlichere
...
beliebige Zielsprachen unterstützen und damit auch solche, die keine
Möglichkeit ähnlich \icode{progn} besitzen, mehrere Ausdrücke zu einem
zusammenzufassen. Deswegen sollten MagicL-Makros beliebig viele \sexps{}
zurückgeben können. Dies passt auch gut zu einer Definition von Makros
als Parser auf \sexps{}, da Parser immer Streams und damit Listen als
Eingaben haben.
\begin{code}
...
Compiler reibungslos funktioniert. Um vernünftig mit diesen Makros
...
(defmacro x ()
  `(defmacro y ()
     `(print ,a)))
\end{code}
ist zunächst nicht klar, auf welches der beiden Quotes sich das Unquote
...
das innere Backquote - soll es sich auf das äußere beziehen, müssen
...
normalen Quote-Operatoren \icode{'}, \icode{,} und \icode{,@} benannte
Operatoren \icode{'1}, \icode{,1} und \icode{,@1} etc. angeboten,
so dass der Bezug immer klar ist, beispielsweise kann zwischen
\icode{(' ('1 (, x)))} und \icode{(' ('1 (,1 x)))} unterschieden
werden. 

Quasiquote und Makrosystem sind sehr eng miteinander verbunden. Deshalb wird
zunächst das erwünschte Makrosystem vorgestellt, bevor beides auf einmal
implementiert wird.

\lsubsection[sexpcomp:spec:comp]{Lisp-artige Makroexpansion}

Die Definition von Lisp-Makros vom Typ \icode{SexpParser [Sexp]} ist mit
den vorhandenen Mitteln bereits möglich:
\begin{code}
...
(mac compPredefine predefine
   ...
   (where ...))
\end{code}
Dies soll in obigen Code übersetzt werden. Hier wird zwischen dem
Compilernamen \icode{compPredefine} und dem Symbolnamen
\icode{predefine} für den Makroaufruf unterschieden, da ersterer eine
Haskell-Variable erzeugt und damit damit Sonderzeichen wie \icode{-}
ausschließt.

Diese Makros unterscheiden sich allerdings in einer Hinsicht von anderen
Lisp-Makros, da der Kontrollfluss hier explizit gesteuert
wird. Lisp-Makrosysteme hingegen besitzen eine globale Liste
von Makros, die auf jeden zu expandierenden Ausdruck angewendet
wird. Trifft kein Makro zu, wird der Knotenname beibehalten und die
Kinder rekursiv expandiert. Derartige Makros, die automatisch in einen
rekursiv absteigenden Compiler integriert werden, sollen mittels
\icode{autoMac} definiert werden können:
\begin{code}
...
Für die Definition eines Compiler-Moduls sollte zudem eine Notation
...
erzeugt, so dass hier auch die Quasiquote-Notation selbst bereits
benutzt werden sollte. Es wird sich daher um einen metarekursiven
Compiler handeln, d.h. einen Compiler, der sich selbst übersetzt. Um
hier das Henne-Ei-Problem zu lösen, wird wie im Compilerbau üblich
später ein Bootstrapping\footnote{Der Begriff Bootstrapping ist eine
  Anspielung auf Baron Münchhausen, der sich an seinen Schnürsenkeln aus
  dem Sumpf gezogen haben will.} erforderlich sein. Metarekursive
Programme sind in der Lisp-Welt nicht unüblich - bereits der erste
LISP\footnote{LISP ist der erste Dialekt der Sprachfamilie
  Lisp.}-Interpreter wurde als \icode{eval}-Funktion in LISP selbst
implementiert und dann manuell in Maschinensprache
...
ein Common Lisp ohne Objekte übersetzt werden kann.

Gehen wir also zunächst davon aus, solch ein Compiler wäre bereits
vorhanden und könnte für die Implementation genutzt werden.


\begin{code}
(=
  compQuote
  (macro (Str ') (>>^ inners single))
    (where 
      ...
      (= inners                     
         (>>> (many (<+> unquoteAll 
              ...
              (arr (namedNode (Str ++)))))
      (= unquote (macro (Str ,) (>>^ take single)))
      (= unquoteAll (macro (Str ,@) take))
      (= (quoteSymbol sym)
...
\lsection[sexpcomp:preproc]{Beispiel: Ein simpler Präprozessor}
