...
\end{code}
Ein derartiger Operator soll nun konstruiert werden. Da dieser aber
speziell auf MagicL zugeschnitten sein soll und keine bloße Kopie
bestehender Lisp-Quasiquotes, müssen zunächst die genauen Anforderungen
diskutiert werden:

\lsubsection[sexpproc:gen:reqs]{Anforderungen}

Da in MagicL bisher
keine syntaktischen Erweiterungen vorgesehen sind, sondern eine rein
\sexp{}-basierte Repräsentation von Quelltext benutzt wird, müssen die
Operatoren wie andere Ausdrücke geklammert werden. Zudem wird nicht
zwischen Quote und Quasiquote unterschieden, so dass einheitlich der
Quote-Operator \icode{'} benutzt wird. Die Funktion \icode{genUser}
sollte sich letztlich wie folgt formulieren lassen:
\begin{code}
(= (genUser userName userEmail)
...
Haskell-Code erzeugt werden kann.

\lsubsection[sexpproc:gen:impl]{Implementation}
...
vorhanden und könnte für die Implementation bereits genutzt werden.
...
(=
  ...
  (where
    ...
    (=
      ...
      (>>>
        (many
          (<+> unquoteAll (>>> inner (arr (namedNode (Str List))))))
        (arr (namedNode (Str ++)))))
    ...
    (=
      (quoteNode nod)
      (++ (List (node (++ (List (symbol (Str node))) (List nod))))))))
\end{code}
