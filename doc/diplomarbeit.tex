...
vorgesehen, können aber über Unterknoten oder spezielle Symbole
dennoch repräsentiert werden. Beispielsweise könnte das Attribut
\icode{color} mit Wert \icode{red} eines Autos durch \icode{(car
  (color red))} oder \icode{(car :color red)} ausgedrückt werden, eine
Liste durch \icode{(list a b c)}.
...
universelle Modellierungssprache an, mit der auch zusätzliche Features wie
Attribute oder Listen indirekt repräsentiert werden können.
...
In der Baumrepräsentation lassen sich Attribute als zusätzlicher
Kantentyp verstehen, so dass das Beispiel
...
\end{code} %"<
durch \abb{xml_tree} visualisiert werden könnte.
...
Es stellt sich also die Frage, ob XML-Attribute überhaupt notwendig
sind oder vielmehr ein optionales Feature, was nur aufgrund der - aus
Sicht eines Lisp-Programmierers - zu umständlichen Syntax für die
Knotendefinition benutzt wird. In diesem Fall sollten Attribute wohl
in der Zukunft aus XML verschwinden, schließlich schreibt das
W3C\cite{rec_xml}: "`The number of optional features in XML is to be
kept to the absolute minimum, ideally zero"'. Wird also ein zweiter,
limitierter Kantentyp für Anwendungen benötigt? 
...
\bf Komponententrennung  &möglich        &stark                       &                           &                &                &         &            \\\hline
