...
in \cref{reqs} Anforderungen für das universelle, im Rahmen dieser Arbeit
entworfene Framework MagicL abgeleitet. \todo{mehr}

\cref{cats} führt in die Kategorientheorie - einen sehr abstrakten Zweig
der Mathematik - ein, mit deren Hilfe - neben vielfältigen anderen
Verwendungsmöglichkeiten - Verarbeitungsprozesse graphisch und
semantisch korrekt beschrieben sowie elegant durch Operatoren kombiniert
werden können. Aus diesem Grund basiert die gesamte Architektur von
MagicL auf Morphismen und Funktoren, was zu einer sehr hohen
Flexibilität des Frameworks führt.

Die Programmiersprache Haskell stellt dem Programmierer mit Arrows und
Monaden Umsetzungen katogorientheoretischer Konzepte zur Verfügung, mit
deren Hilfe es unter anderem gelingt, Nebeneffekte sauber isoliert in
die ansonsten pur funktionale Sprache zu integrieren. MagicL ist aus
diesem Grund in Haskell programmiert, weshalb \cref{haskell} die Sprache
ausführlich vorstellt.

\cref{arrowparser} zeigt, wie sich durch die Kombination einfacher
Funktoren eine Kategorie für Parser konstruieren lässt. Die
Haskell-Umsetzung dieser Kategorie ergibt eine praktische und Bibliothek
für die Erzeugung und Komposition von Parsern. Als Anwendungsbeispiel
dient das Einlesen von \sexps{} aus einer
...
Parser, funktionale und imperative Compiler angesprochen werden
können. Damit wird es möglich, Haskell mittels Typinferenz selbst den
passenden Compiler für eine Aufgabe auswählen zu lassen.
...
Verarbeitung von \sexps{} beschrieben werden kann, so dass Makros hier
als Parser auf \sexps{} aufgefasst werden und damit universeller sind
als Lisp-Makros, welche ausschließlich \sexps{} als Ausgabe liefern
können. Diese werden benutzt, um HSexp, eine in \sexps{} repräsentierte
Haskell-Teilmenge in ein typisiertes Modell zu überführen, welches
anschließend nach Haskell kompiliert wird. Für die Generierung von
\sexps{} wird eine DSL ähnlich Lisp konstruiert, die
Quasiquote-Operatoren enthält, welche besser als bestehende
Lisp-Varianten für die Erzeugung von Meta-Code selbst geeignet
ist. Diese DSL wird metazirkulär nach HSexp und anschließend nach
Haskell übersetzt.
...
\cref{end} fasst die Arbeit zusammen und diskutiert das entstandene
Framework. Anschließend wird im Ausblick auf weitere Verbesserungs- und
Erweiterungsmöglichkeiten eingegangen, die in der Zukunft behandelt
werden könnten.
...
\todo{beschreiben}
Denn es gibt nur eine einzige globale Zuordnungliste von
Knotennamen zu Makros, die bei jedem Makroexpansionsschritt durchsucht
wird. Nach der Expansion wird die gesamte Liste wieder ...

Lisp-Makros können nicht direkt aufgerufen werden, stattdessen wird ein
\sexp{}-Knoten ähnlich wie bei C-Makros als Makroaufruf interpretiert,
wenn der Knotenname mit einem Makronamen übereinstimmt. Hier stellt sich
wieder die Frage, ob die äußere Ununterscheidbarkeit von Makros und
Funktionsaufruf eher positiv oder negativ zu sehen ist. Positiv ist,
...
sind typisierte Datenstrukturen generell von Nöten. Modelldefinitionen
...
\lsection[parsergen]{Parser-Generatoren}
...
\lsubsection[arrowparser:lib:example]{Beispiel: Ein \sexp{}-Parser}
...
\lsection[sepxproc:sexphs]{Metarekursive Kompilation nach Haskell}
