...
Folientitel, Aufzählungen, Bilder etc. in einer kurzen und eleganten
...
graphischen Benutzeroberflächea zu bearbeiten, ist sehr praktisch für
...
JavaScript\cite{JavaScript}, weshalb es direkt von einem JavaScript-Interpreter
eingelesen werden kann. Dies macht JSON vor allem für Webanwendungen
interessant. Mit JSON können anonyme Objekte mit Attributen (auch Listen)
...
zusammensetzen, was im Normalfall zu umständlich ist. Stattdessen wird
eine ``schablonenartige'' Notation bevorzugt, die sich gut anhand der im
Lisp-Umfeld üblichen Operatoren Quasiquote\footnote{Neben Quasiquote ist
  auch der Begriff Backquote üblich. Neben dem Quasiquote gibt es in
  Lisp-Sprachen auch ein einfaches Quote, welches einen komplett
  statischen Bereich erzeugt, ohne darin auf Unquotes zu achten.} und
Unquote beschreiben lässt. Ein Quasiquote erzeugt ein Template, und das
Innere eines Quasiquotes wird als statisch interpretiert, solange darin
kein Unquote auftritt. Ein Unquote, welches nur innerhalb eines
Quasiquote auftauchen darf, erzeugt einen dynamischen Unterabschnitt,
welcher in der vom Werkzeug bereitgestellten Meta-Sprache beschrieben
ist. Hier werden die Eingabeparameter des Templates verwendet. Der
Vorteil dieser Notationsform besteht darin, dass ein Template - bis auf
die dynamischen Abschnitte - genauso aussieht wie das Zieldokument und
dadurch gut lesbar wird.
...
oder XML \seec{model}. Mit den Modellen variieren auch die
...
\lsubsection[template:crit:def]{Template-Erzeugung}
...
quotiert. Manche Werkzeuge zur \cgen{} besitzen kein universelles
...
Unquote-Bereichen ist nicht überall gegeben. Manche Systeme benutzen
auch ein implizites Unquote, welches automatisch eingesetzt wird, wenn
ein Token mit dem Namen eines Template-Parameters übereinstimmt.
...
Template. Manche Werkzeuge bieten neben Templates ebenfalls die
...
modularisierbar und benötigt vergleichsweise kurze
Metaprogramme.
...
von Haskell - ergänzt um die Möglichkeit, über Reification
...
die Anwendung kategorientheoretischer Konzepte erreicht, welche im
...
\todo{Klassen (Vorbedingungen, Multi, Variablen als Konstruktoren, Abhängigkeiten)
 Hier: (...) => als Auslassung}
...
hier ist also \icode{(id,lift)}. Im Vergleicht zu Haskells
...
erfolgreich ist, wird das Ergebnis (lokal) in \icode{x}
gespeichert. Ist daraufhin \icode{bar x} ebenfalls erfolgreich, wird
dieses in \icode{y} gespeichert und schließlich der Wert \icode{Just
  (x, y)} zurückgegeben. Schlägt eine der Funktionen fehl, ist das
Ergebnis \icode{Nothing}.
...
auch eine freie Auswahl der Kategorie ermöglicht, in der Parser ausgeführt
...
Fehler schlecht - stattdessen möchte man nach dem Lesen von \texttt{\{}
...
hilfreich, um teilweise sinnvollere Fehlermeldung zu erhalten.
