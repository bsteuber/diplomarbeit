...
instance (Show t, ArrowFail ar, ArrowChoice ar, Executable (ar [t] a) [t] a) => Executable (ParseFunctor t ar () a) [t] a where
    toIO = toIO . execParser

instance (Show t, ArrowFail ar, ArrowChoice ar, Executable (ar [t] [a]) [t] [a], Compilable (ParseFunctor t ar () a) [t] a) =>
    Compilable (ParseFunctor t ar () [a]) [t] [a] where
        comp = many comp

instance (Show t, ArrowFail ar, ArrowChoice ar, Executable (ar [t] (Maybe a)) [t] (Maybe a), Compilable (ParseFunctor t ar () a) [t] a) => 
...
debug msg = P $ lift $ lift $ Kleisli $ \ x -> do
              putStrLn msg
              return x
